...
describe('Config farms', () => {
  ...
  it.each(farmsToTest)('Farm %d has the correct token addresses', async (pid, farm) => {
    const tokenAddress = farm.token.address[137]
    const quoteTokenAddress = farm.quoteToken.address[137]
  ...
  it.each(farmsToTest)('Farm %d has non 0 tokens amount', async (pid, farm) => {
    const tokenContract = getBep20Contract(farm.token.address[137])
    const quoteTokenContract = getBep20Contract(farm.quoteToken.address[137])
