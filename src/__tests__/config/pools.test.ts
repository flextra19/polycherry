...
describe('Config pools', () => {
  ...
  it.each(poolsToTest.filter((pool) => pool.earningToken.symbol !== 'BNB'))(
    ...
    async (pool) => {
      ...
      expect(rewardTokenAddress.toLowerCase()).toBe(pool.earningToken.address[137].toLowerCase())
    },
  )
  it.each(poolsToTest.filter((pool) => pool.stakingToken.symbol !== 'BNB'))(
    ...
    async (pool) => {
      ...
      expect(stakingTokenAddress.toLowerCase()).toBe(pool.stakingToken.address[137].toLowerCase())
    },
  )
})
