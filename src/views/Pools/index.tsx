...
import { usePools, useFetchCakeVault, useFetchPublicPoolsData, usePollFarmsData, useCakeVault, useGetApiPrices, useFarmFromPid, usePriceCakeBusd } from 'state/hooks'
...
`
interface PropsPools {
  autopool?:boolean
}
const NUMBER_OF_POOLS_VISIBLE = 12

const Pools: React.FC<PropsPools> = (props) => {
  const location = useLocation()
  const { t } = useTranslation()
  const { account } = useWeb3React()
  const prices = useGetApiPrices();
  const {autopool} = props
  ...
  let listPools = poolsToShow()
  if (autopool) {
  listPools = poolsToShow().filter((p) => p.isAutoVault);
  } else {
    listPools = listPools.filter((p) => !p.isAutoVault && p.sousId !== 0);
  }
  ...
  const tableLayout = <PoolsTable pools={listPools} account={account} userDataLoaded={userDataLoaded} />
