...
const ActionPanel: React.FunctionComponent<ActionPanelProps> = ({
  ...
  const bsc = getBscScanAddressUrl(getMasterChefAddress())
  const info = farm.singleToken ? `https://polygonscan.com/token/${farm.token.address[137]}` : `https://polygonscan.com/token/${lpAddress}`
  let addLiquidityUrl
  if (farm.singleToken) {
  ...
  return (
    <Container expanded={expanded}>
      <InfoContainer>
        ...
        <StyledLinkExternal href={info}>{farm.singleToken ? 'See Token Info' : t('See Pair Info')}</StyledLinkExternal>
        <TagsContainer>
          {farm.exchange === "PolyCat" ? <PolyCatSwapTag /> : null }
          {farm.exchange === "QuickSwap" ? <QuickSwapTag /> : null }
          {farm.exchange === "JetSwap" ? <JetSwapTag /> : null }
        </TagsContainer>
      </InfoContainer>
      <ValueContainer>
