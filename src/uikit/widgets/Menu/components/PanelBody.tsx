...
import { FastfoodTwoTone, Timeline, Autorenew, Security } from "@material-ui/icons";
  ...
  return (
    <Container>
      {links.map((entry) => {
        ...
        let iconElement = <Icon width="24px" mr="16px" />;
        const calloutClass = entry.calloutClass ? entry.calloutClass : undefined;
        if (entry.label.includes('HotDog')) {
          iconElement = <FastfoodTwoTone style={{width:'24px', marginRight:'16px'}}  />
        }
        ...
        if (entry.label.includes('Audit')) {
          iconElement = <Security style={{width:'24px', marginRight:'16px'}} />
        }
        if (entry.items) {
